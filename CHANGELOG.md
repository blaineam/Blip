...
### Docs — features and FAQ pages
- **blip.wemiller.com gains `features/` and `faq/`** beside the short landing page, per the portfolio's multi-page model: the full Mac feature catalogue (refreshed — no more "New in v1.5.0" labels, and the speed test described as it works today), the iPhone and iPad features, the eight Shortcuts actions and the numbers; and every FAQ question, grouped, ending with EU availability. The landing nav links Overview · Features · FAQ, each story gets a Learn more link, and the landing FAQ ends with the EU question and a link to the rest. New dictionaries `docs/i18n/{features,faq}.<lang>.json` in all nine languages, reusing the pre-redesign translations.
