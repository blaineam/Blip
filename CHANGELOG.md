...
- **Speed test uses OpenSpeedTest only** — Blip no longer reverse-engineers public web speed tests (the earlier Cloudflare/OVH/Hetzner options are removed). The native test runs solely against an OpenSpeedTest server you point it at — open-source and sanctioned for direct use — so there's no unsanctioned third-party endpoint and no rate-limit surprises. The engine inspects HTTP status and surfaces a clear message on failure; bytes are counted at the chunk level for accurate multi-gig results. OpenSpeedTest is credited in-panel with a one-click "how to self-host" link
- **Public web test (no self-host)** — if you haven't set up your own server, a "Run the public web test" option opens OpenSpeedTest's official hosted test in an embedded web view — the sanctioned way to use their public service. It **auto-starts** and an injected helper **scrapes the completed download/upload result back into Blip's native history/chart**, so even the public test feeds the graph. (Best-effort: it reads the page's result fields, so it could need updating if OpenSpeedTest changes their markup. Self-host for unlimited, fully-private, always-graphed results.)
