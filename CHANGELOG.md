...
- **Public web test (no self-host)** — if you haven't set up your own server, a "Run the public web test" option opens OpenSpeedTest's official hosted test in an embedded web view — the sanctioned way to use their public service. Results live in that window (not Blip's native chart); self-host for graphed, unlimited, fully-private results
