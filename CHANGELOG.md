...
- **Cloudflare download speed test fixed** — Cloudflare rejects `__down` requests ≥100 MB (HTTP 403); the chunk size is now 50 MB, so download throughput measures correctly again. The engine counts bytes at the chunk level via a session delegate for accurate multi-gig results
