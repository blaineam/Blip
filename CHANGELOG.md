...
- **Speed test uses OpenSpeedTest only** — Blip no longer reverse-engineers public web speed tests (the earlier Cloudflare/OVH/Hetzner options are removed). The test runs solely against an OpenSpeedTest server you point it at — open-source and sanctioned for direct use — so there's no unsanctioned third-party endpoint and no rate-limit surprises. The engine inspects HTTP status and surfaces a clear message on failure; bytes are counted at the chunk level for accurate multi-gig results
