...
@MainActor
// iOS PORT of the Mac OpenSpeedTestWidgetRunner (Blip/Sources/Services/) — the widget
// scraping protocol and injected script are IDENTICAL and must stay in sync with the Mac
// copy; only the invisible host differs (a zero-alpha WKWebView attached to the key window
// so WebKit keeps its timers unthrottled, instead of a borderless NSWindow).
final class OpenSpeedTestWidgetRunner: NSObject, WKScriptMessageHandler, WKNavigationDelegate {
    ...
    func run(timeout: TimeInterval = 180,
        ...
        let frame = CGRect(x: 0, y: 0, width: 760, height: 560)
        let wv = WKWebView(frame: frame, configuration: config)
        wv.navigationDelegate = self
        webView = wv

        // Invisible in-hierarchy host: attached to the key window at alpha 0 and inert to
        // touches, so WebKit treats the page as visible and keeps its timers unthrottled —
        // the same trick as the Mac's zero-alpha window, in UIKit clothes.
        wv.alpha = 0
        wv.isUserInteractionEnabled = false
        let scene = UIApplication.shared.connectedScenes.compactMap { $0 as? UIWindowScene }.first
        scene?.keyWindow?.addSubview(wv)
